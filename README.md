...
Sidecar readiness verifies the binary's Ed25519 signature using Fallow 3.22.0's certified signing key and checks adjacent platform-package metadata against 0.4.1. The version is package-declared, not independently attested by a handshake; standalone signed binaries without recognized version metadata remain explicitly unverified, not missing. Readiness does not certify licenses, artifacts, or successful analysis. Existing `o` Runtime Coverage Run still uses only Pi's managed sidecar; external installation use in the inline run flow remains follow-up work.
