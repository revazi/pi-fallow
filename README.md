...
TUI reports now have a persistent view header: **1 Findings**, **2 Similar Code**, and **3 Runtime Coverage**, including empty reports. Switching views keeps the findings selection, filters, expansion, scroll, and prompt detail intact. The optional views currently describe the workflow; inline readiness, configuration, installation, and execution are follow-up work, not active controls or installation-status claims. Opening or switching views performs no analysis or setup.
