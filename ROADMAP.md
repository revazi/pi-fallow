...
- The frozen Apple M1 Pro performance baseline measures the same Fallow 3.7.0 installation at 111.23 ms warm median when invoked directly and 804.50 ms through cached npx fallback. Git ref autocomplete blocks for a 12.14 ms cold median, base detection takes 34.70 ms warm median through three Git processes, and large/schema reports retain roughly 2.43–2.45× their fixture size on the heap. These values are machine-specific and must be compared in a matching environment.
